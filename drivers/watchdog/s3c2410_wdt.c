...
{
	return __raw_readl(wdt_base + S3C2410_WTCON) & S3C2410_WTCON_ENABLE;
}
...
static int s3c2410wdt_set_heartbeat(struct watchdog_device *wdd, unsigned timeout)
{
	...
	*/
	...
	__raw_writel(count, wdt_base + S3C2410_WTDAT);
	__raw_writel(wtcon, wdt_base + S3C2410_WTCON);
...
static int s3c2410wdt_resume(struct platform_device *dev)
{
	...
	__raw_writel(wtdat_save, wdt_base + S3C2410_WTDAT);
	__raw_writel(wtdat_save, wdt_base + S3C2410_WTCNT); /* Reset count */
	__raw_writel(wtcon_save, wdt_base + S3C2410_WTCON);

	pr_debug(PFX "watchdog %sabled\n",
