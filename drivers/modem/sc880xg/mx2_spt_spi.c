...
static inline void spt_spi_mrdy_set(struct spt_spi_device *spt_dev, bool active)
{
	...
	MDM_DEBUG("%s: %d:%d\n", __func__, active, gpio_get_value(spt_dev->gpio.mrdy));
}
/**
...
static inline void spt_spi_mrts_set(struct spt_spi_device *spt_dev, bool active)
{
	...
	MDM_DEBUG("%s: %d:%d\n", __func__, active, gpio_get_value(spt_dev->gpio.mrts));
}
/**
...
static inline void spt_spi_mrsd_set(struct spt_spi_device *spt_dev, bool active)
{
	...
	MDM_DEBUG("%s: %d:%d\n", __func__, active, gpio_get_value(spt_dev->gpio.mrsd));
}
/**
...
{
	int val = spt_spi_srdy_get(spt_dev) || spt_spi_srts_get(spt_dev);
...
{
	struct spt_spi_device *spt_dev = tty->driver_data;
	MDM_DEBUG("%s : count=%d\n", __func__, SPT_SPI_TXFIFO_SIZE - kfifo_len(&spt_dev->tx_fifo));
...
static irqreturn_t spt_spi_srdy_interrupt(int irq, void *dev)
{
	...
	int val = spt_spi_srdy_get(spt_dev) | spt_spi_srts_get(spt_dev)<<1;
