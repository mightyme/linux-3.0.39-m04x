...
#define NO_SLEW_CONTROL
//#undef NO_SLEW_CONTROL

#ifdef DEBUG_PMIC
#define pmic_pr_debug(fmt, args...) printk(PMIC_REG_DEBUG pr_fmt(fmt), ## args)
#else
#define pmic_pr_debug(args...) do { } while (0)
#endif
...
static inline int max77686_set_gpio_dvs(struct max77686_data *max77686)
{
	int set3 = ((max77686->buck234_gpioindex) >> 2) & 0x1;
	int set2 = ((max77686->buck234_gpioindex) >> 1) & 0x1;
	int set1 = (max77686->buck234_gpioindex) & 0x1;
	int select_index = max77686->buck_selbindex - MAX77686_BUCK2;
	...
	if (max77686->buck234_gpios_selb[select_index]) {
		gpio_set_value(max77686->buck234_gpios_selb[select_index], 0x0);
		gpio_set_value(max77686->buck234_gpios_selb[select_index], 0x1);
	}
		...
		int *reg, int *mask, int *pattern)
{
	...
	pmic_pr_debug("%s: rid=%d, mode=%d, size=%d\n",
...
static int max77686_reg_is_enabled(struct regulator_dev *rdev)
{
	...
	pmic_pr_debug("%s: id=%d, ret=%d, val=%x, mask=%x, pattern=%x\n",
...
static int max77686_get_voltage(struct regulator_dev *rdev)
{
	...
	pmic_pr_debug("%s: id=%d, reg=%x, mask=%x, val=%x\n",
		...
		int min_uV, int max_uV, unsigned *selector)
{
	...
	if (MAX77686_BUCK1 == rid) {
		org = i << shift;
		ret = max77686_write_reg(i2c, reg, org);
	} else {
	...
	*selector = i;
	...
	pmic_pr_debug("%s: id=%d, reg=%x, mask=%x, org=%x, val=%x\n",
				    ...
				    int min_uV, int max_uV, unsigned *selector)
{
...
#ifndef NO_SLEW_CONTROL
	static unsigned int buck2_voltage_prev = 600000;
	static unsigned int buck3_voltage_prev = 600000;
	static unsigned int buck4_voltage_prev = 600000;
	unsigned int buck_diff = 0, tmp, delay = 0;
#endif
	...
	case MAX77686_BUCK1:
	case MAX77686_BUCK5 ... MAX77686_BUCK9:
		return max77686_set_voltage(rdev, min_uV, max_uV, selector);	
	}
	...
	case MAX77686_BUCK2:
		if(max77686->buck2_gpiodvs){
			for (i=0; i<8; i++) {
				if (max77686->buck2_vol[i] == new_val) {
					max77686->buck234_gpioindex = i;
					max77686->buck_selbindex = MAX77686_BUCK2;
#ifndef NO_SLEW_CONTROL
					tmp = desc->min + desc->step*new_val;
					buck_diff = tmp > buck2_voltage_prev ? (tmp - buck2_voltage_prev) : 0;
					delay = DIV_ROUND_UP(buck_diff, max77686->ramp_delay*1000);
					buck2_voltage_prev = tmp;
#endif					
					break;
				}
			}
			WARN(8 == i, "MAX77686_BUCK2 value = %d\n", min_uV);
		}
		break;
	case MAX77686_BUCK3:
		if(max77686->buck3_gpiodvs){
			for (i=0; i<8; i++) {
				if (max77686->buck3_vol[i] == new_val) {
					max77686->buck234_gpioindex = i;
					max77686->buck_selbindex = MAX77686_BUCK3;
#ifndef NO_SLEW_CONTROL
					tmp = desc->min + desc->step*new_val;
					buck_diff = tmp > buck3_voltage_prev ? (tmp - buck3_voltage_prev) : 0;
					delay = DIV_ROUND_UP(buck_diff, max77686->ramp_delay*1000);
					buck3_voltage_prev = tmp;
#endif					
					break;
				}
			}
			WARN(8 == i, "MAX77686_BUCK3 value = %d\n", min_uV);
		}
		break;
	case MAX77686_BUCK4:
		if(max77686->buck4_gpiodvs){
			for (i=0; i<8; i++) {
				if (max77686->buck4_vol[i] == new_val) {
					max77686->buck234_gpioindex = i;
					max77686->buck_selbindex = MAX77686_BUCK4;
#ifndef NO_SLEW_CONTROL
					tmp = desc->min + desc->step*new_val;
					buck_diff = tmp > buck4_voltage_prev ? (tmp - buck4_voltage_prev) : 0;
					delay = DIV_ROUND_UP(buck_diff, max77686->ramp_delay*1000);
					buck4_voltage_prev = tmp;
#endif					
					break;
				}
			}
			WARN(8 == i, "MAX77686_BUCK4 value = %d\n", min_uV);
		}
		break;
	}

	if (i<8) {	/* We found appropriate voltage by gpio table */
		max77686_set_gpio_dvs(max77686);
		*selector = new_val;
#ifndef NO_SLEW_CONTROL
...
int max77686_update_dvs_voltage(struct regulator_dev *rdev,
				const int *volt_table, const unsigned int size)
{
	...
	BUG_ON(size > 8 || NULL == volt_table);

	switch (rid) {
	case MAX77686_BUCK2:
		pr_info("%s: MAX77686_BUCK2\n", __func__);
		reg = MAX77686_REG_BUCK2DVS1;
		buck_vol = max77686->buck2_vol;
		break;
	case MAX77686_BUCK3:
		pr_info("%s: MAX77686_BUCK3\n", __func__);
		reg = MAX77686_REG_BUCK3DVS1;
		buck_vol = max77686->buck3_vol;
		break;
	case MAX77686_BUCK4:
		pr_info("%s: MAX77686_BUCK4\n", __func__);
		reg = MAX77686_REG_BUCK4DVS1;
		buck_vol = max77686->buck4_vol;
		break;
	default:
		pr_err("%s: Not supported BUCK %d", __func__,rid);
		return -EINVAL;
	}

	for (i = 0; i < size; i++) {
		buck_vol[i] = max77686_get_voltage_proper_val(desc,
					 volt_table[i], volt_table[i] + desc->step);
		ret = max77686_write_reg(i2c, reg+i, buck_vol[i]);
		if (ret) {
			pr_err("%s: max77686_write_reg erro!\n", __func__);
			return ret;
		}
	}
...
EXPORT_SYMBOL(max77686_update_dvs_voltage);
...
static int max77686_buck_suspend_disable(struct regulator_dev *rdev)
{
	struct max77686_data *max77686 = rdev_get_drvdata(rdev);
	struct i2c_client *i2c = max77686->iodev->i2c;
	int ret, reg, mask, pattern;
...
static struct regulator_ops max77686_buck_ops = {
	...
	.set_suspend_enable	= max77686_reg_do_nothing,
	.set_suspend_disable	= max77686_buck_suspend_disable,
};
...
static __devinit int max77686_pmic_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < 3; i++) {
		...
		snprintf(buf0[i], sizeof(buf0[0]), "MAX77686 SELB%d", i);
		if (gpio_is_valid(pdata->buck234_gpio_selb[i])) {
			max77686->buck234_gpios_selb[i] =
				pdata->buck234_gpio_selb[i];
			gpio_request_one(pdata->buck234_gpio_selb[i], GPIOF_OUT_INIT_HIGH, buf0[i]);
		} else {
			dev_info(&pdev->dev, "GPIO %s ignored (%d)\n",
				 buf0[i], pdata->buck234_gpio_dvs[i]);
		}

		snprintf(buf1[i], sizeof(buf1[0]), "MAX77686 DVS%d", i);
		if (gpio_is_valid(pdata->buck234_gpio_dvs[i])) {
			max77686->buck234_gpios_dvs[i] =
				pdata->buck234_gpio_dvs[i];
			gpio_request_one(pdata->buck234_gpio_dvs[i], GPIOF_OUT_INIT_HIGH, buf1[i]);
		} else {
			dev_info(&pdev->dev, "GPIO %s ignored (%d)\n",
				 buf1[i], pdata->buck234_gpio_dvs[i]);
		}
	}
