...
/* flash and movie current, in uA */
static const struct voltage_map_desc flash_vol_cur_map_desc = {
	.min = 23440,	.step = 23440,	.max = 750080,.n_bits = 5,
};
static const struct voltage_map_desc movie_vol_cur_map_desc = {
	.min = 15625,	.step = 15625,	.max = 250000, .n_bits = 4,
};
...
static const struct voltage_map_desc *reg_voltage_map[] = {
	...
	[MAX8997_FLASH_CUR] = &flash_vol_cur_map_desc,
	[MAX8997_MOVIE_CUR] = &movie_vol_cur_map_desc,	
};
...
static inline void max8997_set_gpio(struct regulator_dev *rdev)
{
	struct max8997_data *max8997 = rdev_get_drvdata(rdev);
	int rid = max8997_get_rid(rdev);
	const struct voltage_map_desc *desc = reg_voltage_map[rid];
	int rampup_us = 100;
	int set1 = (max8997->buck125_gpioindex) & 0x1;
	int set2 = ((max8997->buck125_gpioindex) >> 1) & 0x1;
	int set3 = ((max8997->buck125_gpioindex) >> 2) & 0x1;
		
	if (max8997->pre_gpioindex == max8997->buck125_gpioindex)
		return;
	
	pr_debug ("%s: idx is %d --> %d\n", __func__, max8997->pre_gpioindex, max8997->buck125_gpioindex);
	...
	if ( max8997->pre_gpioindex < max8997->buck125_gpioindex ) {
		...
		switch (rid) {
		case MAX8997_BUCK1:
			rampup_us = (max8997->buck1_vol[max8997->buck125_gpioindex] - max8997->buck1_vol[max8997->pre_gpioindex])*desc->step/max8997->ramp_delay;
			break;
		case MAX8997_BUCK2:
			rampup_us = (max8997->buck2_vol[max8997->buck125_gpioindex] - max8997->buck2_vol[max8997->pre_gpioindex])*desc->step/max8997->ramp_delay;
			break;
		case MAX8997_BUCK5:
			rampup_us = (max8997->buck5_vol[max8997->buck125_gpioindex] - max8997->buck5_vol[max8997->pre_gpioindex])*desc->step/max8997->ramp_delay;
			break;
		}		
		rampup_us += 20;
		udelay(rampup_us);
	...
	max8997->pre_gpioindex = max8997->buck125_gpioindex;
}
...
/*Update dvs voltage table for cpufreq performance*/
int max8997_update_dvs_voltage(struct regulator_dev *rdev,
				const int *volt_table, const unsigned int size)
{
	...
	if ( size > 8 ) {
		pr_err ("%s: volt_table size is invalid\n", __func__);
		return -EINVAL;
	}	
	...
	case MAX8997_BUCK1:
		for (i=0; i<size; i++) {
			max8997->buck1_vol[i] =  ret =
				...
				volt_table[i] / 1000,volt_table[i] / 1000 +
				buck1245_voltage_map_desc.step);
	
			ret = max8997_update_reg(i2c, MAX8997_REG_BUCK1DVS1 + i, 
					max8997->buck1_vol[i], 0x3f);
			if (ret)
				printk("%s MAX8997_REG_BUCK1DVS1 write error!\n", __func__);

			pr_debug ("%s:volt:%d, idx :0x%x\n", __func__, 
					volt_table[i], max8997->buck1_vol[i]);
		}
	...
	case MAX8997_BUCK2:
		for (i=0; i<size; i++) {
			...
			max8997_update_reg(i2c, MAX8997_REG_BUCK2DVS1 + i, max8997->buck2_vol[i], 0x3f);
			pr_debug ("%s:volt:%d, idx :0x%x\n", __func__, 
					volt_table[i], max8997->buck2_vol[i]);
		}
...
EXPORT_SYMBOL(max8997_update_dvs_voltage);
...
static int max8997_flash_is_enabled(struct regulator_dev *rdev)
{
	...
	switch (max8997_get_rid(rdev)) {
	case MAX8997_FLASH_CUR:
		mask = MAX8997_FLASH_EN_MASK << MAX8997_FLASH_EN_SHIFT;
		break;
	case MAX8997_MOVIE_CUR:
		mask = MAX8997_MOVIE_EN_MASK << MAX8997_MOVIE_EN_SHIFT;
		break;
	default:
		return -EINVAL;
	}

	ret = max8997_read_reg(i2c, MAX8997_REG_LEN_CNTL, &val);
	if (ret < 0)
		return ret;
...
static int max8997_flash_enable(struct regulator_dev *rdev)
{
	...
	switch (max8997_get_rid(rdev)) {
	case MAX8997_FLASH_CUR:
		val = MAX8997_FLASH_EN_MASK << MAX8997_FLASH_EN_SHIFT;
		mask = MAX8997_FLASH_EN_MASK << MAX8997_FLASH_EN_SHIFT;
		break;
	case MAX8997_MOVIE_CUR:
		val = MAX8997_MOVIE_EN_MASK << MAX8997_MOVIE_EN_SHIFT;
		mask = MAX8997_MOVIE_EN_MASK << MAX8997_MOVIE_EN_SHIFT;
		break;
	default:
		return -EINVAL;
	...
	ret = max8997_update_reg(i2c, MAX8997_REG_LEN_CNTL,
		val, mask);
 ...
 */
int max8997_clear_flash_interrupts(struct i2c_client *i2c)
{
	u8 val;
	int ret;
	ret = max8997_read_reg(i2c, MAX8997_REG_FLASHSTATUS, &val);
...
static int max8997_flash_disable(struct regulator_dev *rdev)
{
	...
	max8997_clear_flash_interrupts(i2c);
...
{
	const struct voltage_map_desc *desc;
	int co = max8997_get_rid(rdev);
	int val;

	if (co >= ARRAY_SIZE(reg_voltage_map))
		return -EINVAL;

	desc = reg_voltage_map[co];
	if (desc == NULL)
		return -EINVAL;
				     ...
				     int min_uA, int max_uA)
{
	...
	if (desc->min + desc->step*i > desc->max)
		return -EINVAL;
