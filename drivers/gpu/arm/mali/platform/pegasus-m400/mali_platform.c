...
mali_io_address clk_pegasus_register_map=0;

_mali_osk_lock_t *mali_dvfs_lock = 0;

#ifdef CONFIG_REGULATOR
int mali_pegasus_regulator_get_usecount(void)
{
	struct regulator_dev *rdev;

	if( IS_ERR_OR_NULL(g3d_regulator) ) {
		MALI_DEBUG_PRINT(1, ("error on mali_pegasus_regulator_get_usecount : g3d_regulator is null\n"));
		return 0;
	}
...
void mali_pegasus_regulator_disable(void)
{
	bPoweroff = 1;
	if( IS_ERR_OR_NULL(g3d_regulator) ) {
		MALI_DEBUG_PRINT(1, ("error on mali_pegasus_regulator_disable : g3d_regulator is null\n"));
		return;
	}
...
void mali_pegasus_regulator_enable(void)
{
	bPoweroff = 0;
	if( IS_ERR_OR_NULL(g3d_regulator) ) {
		MALI_DEBUG_PRINT(1, ("error on mali_pegasus_regulator_enable : g3d_regulator is null\n"));
		return;
	}
...
unsigned long mali_pegasus_clk_get_rate(void)
{
	return clk_get_rate(mali_clock);
}

mali_bool mali_pegasus_clk_get(mali_bool bis_vpll)
{
...
void mali_pegasus_clk_put(mali_bool binc_mali_clock)
{
...
mali_bool mali_pegasus_clk_set_rate(unsigned int clk, unsigned int mhz)
{
...
#if !MALI_DVFS_ENABLED
	clk = mali_gpu_clk = 440;
#endif

	_mali_osk_lock_wait(mali_dvfs_lock, _MALI_OSK_LOCKMODE_RW);

	if (mali_pegasus_clk_get(bis_vpll) == MALI_FALSE)
		return MALI_FALSE;
...
static mali_bool init_mali_pegasus_clock(void)
{
	...
	if (mali_pegasus_clk_set_rate(mali_gpu_clk, GPU_MHZ) == MALI_FALSE) {
		ret = MALI_FALSE;
		goto err_clock_get;
	}

	MALI_PRINT(("init_mali_pegasus_clock mali_clock %p \n", mali_clock));
	...
	regulator_enable(g3d_regulator);
	MALI_DEBUG_PRINT(1, ("= regulator_enable -> use cnt: %d \n",mali_pegasus_regulator_get_usecount()));
	mali_pegasus_regulator_set_voltage(mali_gpu_vol, mali_gpu_vol);
#endif
...
err_clock_get:
	mali_pegasus_clk_put(MALI_TRUE);

	return ret;
}

static mali_bool deinit_mali_pegasus_clock(void)
{
	if (mali_clock == 0)
		return MALI_TRUE;
...
static _mali_osk_errcode_t enable_mali_pegasus_clocks(void)
{
	int err;
...
#if MALI_DVFS_ENABLED
	// set clock rate
	if (get_mali_pegasus_dvfs_control_status() != 0 || mali_gpu_clk >= mali_runtime_resume.clk)
		mali_pegasus_clk_set_rate(mali_gpu_clk, GPU_MHZ);
	else {
		mali_pegasus_regulator_set_voltage(mali_runtime_resume.vol, mali_runtime_resume.vol);
		mali_pegasus_clk_set_rate(mali_runtime_resume.clk, GPU_MHZ);
		if (mali_gpu_clk <= mali_runtime_resume.clk)
			set_mali_pegasus_dvfs_current_step(5);
...
static _mali_osk_errcode_t disable_mali_pegasus_clocks(void)
{
...
void set_mali_pegasus_parent_power_domain(struct platform_device* dev)
{
#if MALI_PMM_RUNTIME_JOB_CONTROL_ON
#if LINUX_VERSION_CODE <= KERNEL_VERSION(2,6,36)
...
_mali_osk_errcode_t pegasus_g3d_power_domain_control(int bpower_on)
{
	if (bpower_on) {
#if MALI_PMM_RUNTIME_JOB_CONTROL_ON
...
_mali_osk_errcode_t mali_pegasus_platform_powerdown(u32 cores)
{
	MALI_DEBUG_PRINT(3,("power down is called in mali_pegasus_platform_powerdown state %x core %x \n", gpu_power_state, cores));
	...
	if (gpu_power_state != 0) {// power down after state is 0
		...
		if (gpu_power_state == 0) {
			...
			disable_mali_pegasus_clocks();
		}
	} else {
		MALI_PRINT(("mali_pegasus_platform_powerdown gpu_power_state == 0 and cores %x \n", cores));
	}
...
_mali_osk_errcode_t mali_pegasus_platform_powerup(u32 cores)
{
	MALI_DEBUG_PRINT(3,("power up is called in mali_pegasus_platform_powerup state %x core %x \n", gpu_power_state, cores));

	if (gpu_power_state == 0) {// power up only before state is 0
		gpu_power_state = gpu_power_state | cores;
		if (gpu_power_state != 0) {
			MALI_DEBUG_PRINT(4,("enable clock \n"));
			enable_mali_pegasus_clocks();
		}
	} else {
		gpu_power_state = gpu_power_state | cores;
	}
...
void mali_pegasus_gpu_utilization_handler(u32 utilization)
{
	if (bPoweroff==0) {
#if MALI_DVFS_ENABLED
		if(!mali_pegasus_dvfs_handler(utilization))
...
u32 pmu_pegasus_get_power_up_down_info(void)
{
	return 4095;
}
...
_mali_osk_errcode_t mali_pegasus_platform_power_mode_change(mali_power_mode power_mode)
{
    MALI_SUCCESS;
}
...
int mali_voltage_lock_deinit(void)
{
	mali_vol_lock_flag = MALI_FALSE;
