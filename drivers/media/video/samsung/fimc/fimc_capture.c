...
/*
 * for calculate the window cut offset, add by jerrymo 
 */
static int fimc_cal_camera_output_size(struct fimc_control *ctrl)
{
	...
	if(o_rate < i_rate)
	{
		width = ctrl->cam->width;
		do{
			width -= 16;
			i_rate = width*1000/ctrl->cam->height;
		}while(o_rate < i_rate);
		r_offset = ctrl->cam->width - width;
		
	}else if(o_rate > i_rate){
		height = ctrl->cam->height;
		do{
			height -= 16;
			i_rate = ctrl->cam->width*1000/height;
		}while(o_rate > i_rate);
...
*/
	...
	ctrl->cam->window.left = left;
	ctrl->cam->window.top = top;
	ctrl->cam->window.height = height;
	ctrl->cam->window.width=  width;

	printk("%s: camera window left=%d, top=%d, width=%d, height=%d\n", __func__, ctrl->cam->window.left, ctrl->cam->window.top, ctrl->cam->window.width, ctrl->cam->window.height);
...
int fimc_s_input(struct file *file, void *fh, unsigned int i)
{
	...
	*/
	if (!ctrl->cap) {
		...
		memset(ctrl->cap, 0, sizeof(struct fimc_capinfo));
	}	
		...
		if (fimc_cam_use) {
			if (ctrl->cam->sd)
				ret = v4l2_subdev_call(ctrl->cam->sd,
							core, s_ctrl, c);
			else if (ctrl->is.sd && ctrl->cam->use_isp)
...
#ifdef CONFIG_BUSFREQ_OPP
	if (fimc_cam_use && !soc_is_exynos4210()) {
		...
		fimc_dbg("0x11a00400 = 0x%x , 0x11a00404 = 0x%x \n", 
