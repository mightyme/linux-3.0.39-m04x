...
u8 mhl_onoff_ex(bool onoff)
{
	...
	if (onoff) {
		...
		if (sii9234->pdata->hw_onoff)
			sii9234->pdata->hw_onoff(sii9234->pdata,0);
			   ...
			   u8 *value)
{
	...
	if (!value)
		return -EINVAL;
	//pr_info("[MHL]mhl_tx_read_reg read addr %x",sii9234->pdata->mhl_tx_client->addr*2);
			 ...
			 u8 value)
{
	...
	ret = i2c_smbus_write_byte_data(sii9234->pdata->tpi_client, offset,
					value);
	if (ret < 0)
		pr_err("[ERROR] sii9234 : %s(0x%02x, 0x%02x) errno\n", __func__,
		       offset, value);
	return ret;
}
...
static int mhl_wake_toggle(struct sii9234_data *sii9234,
			   unsigned long first_period, unsigned long second_period)
{
	int ret;
	u8 value;
	
	ret = mhl_tx_read_reg(sii9234, MHL_TX_DISC_CTRL7_REG, &value);
	if (ret < 0)
		goto exit;	
	value |= 0xC0;
	ret = mhl_tx_write_reg(sii9234, MHL_TX_DISC_CTRL7_REG, value);
	if (ret < 0)
		goto exit;
    ...
    mdelay(second_period);
	return 0;
exit:
	pr_err("[MHL] i2c error!\n");
	return ret;
}
...
{
	int ret;
	pr_info("[MHL]: CbusWakeUpPulseGenerator: I2C mode\n");
	// Start the pulse
...
static int sii9234_cbus_init(struct sii9234_data *sii9234)
{
	...
	ret = cbus_write_reg(sii9234, 0x07, /*0x32*/0xF2);
	if (ret < 0)
		goto i2c_error_exit;
	...
	/* Setup our devcap */
	ret = cbus_write_reg(sii9234, 0x80 + DEVCAP_DEV_STATE, 0x04);
				/*To meet cts 6.3.10.1 spec */
	if (ret < 0)
		goto i2c_error_exit;
	ret = cbus_write_reg(sii9234, 0x80 + DEVCAP_MHL_VERSION, 0x10);
				/*mhl version 1.0 */
	if (ret < 0)
		goto i2c_error_exit;
	ret = cbus_write_reg(sii9234, 0x80 + DEVCAP_DEV_CAT, 0x00);
	if (ret < 0)
		goto i2c_error_exit;
	ret = cbus_write_reg(sii9234, 0x80 + DEVCAP_ADOPTER_ID_H, 0x01);
	if (ret < 0)
		goto i2c_error_exit;
	ret = cbus_write_reg(sii9234, 0x80 + DEVCAP_ADOPTER_ID_L, 0x42);
	if (ret < 0)
		goto i2c_error_exit;
	...
	if (ret < 0)
		goto i2c_error_exit;
	ret = cbus_write_reg(sii9234, 0x80 + DEVCAP_LOG_DEV_MAP, (1<<1 | 1<<2 | 1<<3 | 1<<7));
	if (ret < 0)
		goto i2c_error_exit;
	ret = cbus_write_reg(sii9234, 0x80 + DEVCAP_BANDWIDTH, 0);
	if (ret < 0)
		goto i2c_error_exit;
	ret =
	...
	if (ret < 0)
		goto i2c_error_exit;
	ret = cbus_write_reg(sii9234, 0x80 + DEVCAP_DEVICE_ID_H, 0x92);
	if (ret < 0)
		goto i2c_error_exit;
	ret = cbus_write_reg(sii9234, 0x80 + DEVCAP_DEVICE_ID_L, 0x44);
	if (ret < 0)
		goto i2c_error_exit;
	ret = cbus_write_reg(sii9234, 0x80 + DEVCAP_SCRATCHPAD_SIZE, 0x10);
	if (ret < 0)
		goto i2c_error_exit;
	ret = cbus_write_reg(sii9234, 0x80 + DEVCAP_INT_STAT_SIZE, 0x03);
	if (ret < 0)
		goto i2c_error_exit;
	...
	if (ret < 0)
		goto i2c_error_exit;
	value |= 0x0C;
	ret = cbus_write_reg(sii9234, 0x22, value & 0x0F);
	if (ret < 0)
		goto i2c_error_exit;    
...
static int sii9234_mhl_tx_ctl_int(struct sii9234_data *sii9234)
{
	...
	if (ret < 0)
		goto i2c_error_exit;
#ifdef	/*NONE_SENSE*/__CONFIG_RSEN_LOST_PATCH__
...
/*
static void mask_interrupt(bool mask)
{
	struct sii9234_data *sii9234 = dev_get_drvdata(sii9244_mhldev);
	if(mask){
...
static void sii9234_clear_intr(void)
{
	struct sii9234_data *sii9234 = NULL;
	int ret;
	u8 intr1, intr4, value;
	u8 intr1_en, intr4_en;
	u8 cbus_intr1, cbus_intr2;
	sii9234 = dev_get_drvdata(sii9244_mhldev);
	
	pr_info("%s() show interrupt enable/status regs %d\n",__func__);
	if (!sii9234) {
	...
	pr_debug("clearing interrupts before entering d3 mode.\n");
	...
	if (ret < 0)
		goto i2c_error_exit;
	pr_debug("after clear interrupt\n");
	mhl_tx_read_reg(sii9234, MHL_TX_INTR4_REG, &intr4);
	pr_debug("%2x\n",intr4);
	sii9234_mutex_unlock(&sii9234->lock);
	return;
i2c_error_exit:
	pr_err("[MHL]%s() i2c error\n", __func__);
	sii9234_mutex_unlock(&sii9234->lock);
...
//		mhl_hpd_handler(false);
	...
	ret = mhl_tx_write_reg(sii9234, MHL_TX_DISC_CTRL2_REG, 0xA5);
	if (ret < 0)
		goto exit;
...
//different begin
	...
	if (ret < 0)
		goto exit;
/*		
	ret = mhl_tx_set_reg(sii9234, 0x91, BIT3);
	if (ret < 0)
		goto exit;
*/		
	ret = mhl_tx_clear_reg(sii9234, 0x96, BIT5);	// Enable CI2CA as an open-drain output (to control external USB switch)
	if (ret < 0)
		goto exit;
	//different end
	...
	ret = mhl_tx_clear_reg(sii9234, MHL_TX_INT_CTRL_REG, (1 << 6) |(1 << 2) | (1 << 1));
	if (ret < 0)
		goto exit;
	...
	if (ret < 0)
		goto exit;
	//the following part is the same with switch_to_d3() in 9244
...
static int sii9234_detection_callback(void)
{
	...
	pr_debug("%s() function called- start\n", __func__);
...
static irqreturn_t sii9234_irq_thread(int irq, void *data)
{
	...
	sii9234 = dev_get_drvdata(sii9244_mhldev);
	pr_info("%s() interrupt triggered irq is %d\n",__func__,irq);
	if (!sii9234) {
		pr_err("[ERROR] %s: sii9234 is NULL & skipp this rutine\n",
		       __func__);
		return IRQ_HANDLED;
	...
	if (intr4 & RGND_READY_INT) {
		//mask_all_except_rgnd_intr(false);
	...
	if (intr4 & MHL_EST_INT) {	/* 1<<2 */
		...
		/*
		ret = hdmi_rx_write_reg(sii9234, 0x01, 0x00);
		if(ret < 0) {
				printk(KERN_ERR
			       "[ERROR] %s():%d hdmi_rx_write_reg failed !\n",
			       __func__, __LINE__);
			goto i2c_error_exit;
		}
		*/
...
//			mhl_hpd_handler(true);
	...
	if (intr1 & RSEN_CHANGE_INT) {
		...
		if (value & RSEN_STATUS) {
			...
			if ((value & RSEN_STATUS) == 0) {
				printk(KERN_INFO
				       "%s() RSEN Really LOW ~\n", __func__);
				//mask_all_except_rgnd_intr(true);
 ...
 err_exit:
	pr_debug("TAG err_exit sii9234: wake_up\n");
	wake_up(&sii9234->wq);
 err_exit2:
	ret = mhl_tx_write_reg(sii9234, MHL_TX_INTR1_REG, intr1);
	if (ret < 0)
 ...
 i2c_error_exit:
	...
	pr_debug("TAG i2c_err_exit sii9234: wake_up\n");
	wake_up(&sii9234->wq);
	return IRQ_HANDLED;
}
...
#endif
static void sii9234_show_int_regs(void)
{
	struct sii9234_data *sii9234 = NULL;
	int ret;
	u8 intr1, intr4;
	...
	pr_info("%s() show interrupt enable/status regs \n",__func__);
	if (!sii9234) {
		pr_err("[ERROR] %s: sii9234 is NULL & skipp this rutine\n",
		       __func__);	
	}
...
static ssize_t sii9234_store(struct device *dev, 
			     struct device_attribute *attr,
			     const char *buf, size_t count)
{
	//sii9234_irq_thread(0,0);
	...
	if(buf[0]-'0'==1)
		...
		sii9234_show_int_regs();
	return 0;
	
}

DEVICE_ATTR(sii9234_onoff,S_IRUGO | S_IWUSR,NULL,sii9234_store);
static int sii9234_create_attrs(struct device * dev)
{
	int rc;
	rc = device_create_file(dev, &dev_attr_sii9234_onoff);	
	return rc;

}

static int __devinit sii9234_mhl_tx_i2c_real_probe(struct i2c_client *client)
{
	...
	mhl_dbg_flag = 1;
#endif
	pr_info("[MHL] sii9244 probe start\n");
	pdata = client->dev.platform_data;		
	if(pdata->cbus_client == NULL ||
		pdata->tpi_client == NULL || 
		pdata->hdmi_rx_client == NULL||
		pdata->mhl_tx_client == NULL){
		return -1;
	}
	pr_info("[MHL] sii9244 probe\n");
	...
	sii9234_create_attrs(&pdata->mhl_tx_client->dev);
	client->irq = pdata->eint;
	pdata->mhl_tx_client->irq = pdata->eint;
	i2c_set_clientdata(client, sii9234);
	sii9244_mhldev = &client->dev;
	dev_set_drvdata(sii9244_mhldev, sii9234);	
					   ...
					   const struct i2c_device_id *id)
{
	...
	sii9234_mhl_tx_i2c_real_probe(client);
	return 0;
}
