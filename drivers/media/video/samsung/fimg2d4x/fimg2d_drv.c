...
void fimg2d_early_suspend(struct early_suspend *h);
void fimg2d_late_resume(struct early_suspend *h);
#endif
...
static struct miscdevice fimg2d_dev = {
	.minor		= MISC_DYNAMIC_MINOR/*FIMG2D_MINOR*/,
	.name		= "fimg2d",
	.fops		= &fimg2d_fops,
};
...
//	exynos_sysmmu_set_fault_handler(info->dev, fimg2d_sysmmu_fault_handler);
	...
	if (IS_ERR_OR_NULL(info->bus_dev)) {
		pr_err("%s: failed to dev_get exynos-busfreq\n", __func__);
		goto err_opp;
	}
#endif
...
static void fimg2d_shutdown(struct platform_device *pdev)
{
	int timeout=100;
	atomic_set(&info->suspended, 1);
	while (1) {
		if (fimg2d_queue_is_empty(&info->cmd_q) || timeout<0)
			break;

		msleep(2);
	}
	fimg2d_debug("shutdown... done\n");
}
...
static int __init fimg2d_register(void)
{
	if(machine_is_m030())
	{
		printk("%s: platform device register Failed(machine don't match)\n", __func__);
		return -1;
	}
