...
static void check_adb_lock(void);
#endif
struct adb_dev {
...
#else
static int register_max8997_charger_notifier(struct notifier_block *nb){return 0;}
#endif
#ifdef CONFIG_CHARGER_MAX77665
extern int register_max77665_charger_notifier(struct notifier_block *nb);
#else
...
{
	if(machine_is_m030())
		return register_max8997_charger_notifier(nb);
	else
		return register_max77665_charger_notifier(nb);
}
...
static int charger_event(struct notifier_block *this, unsigned long event, void *ptr)
{
...
{
	.notifier_call = charger_event,
};

static void adb_wake_lock_init(void)
{
#ifdef CONFIG_WAKELOCK
