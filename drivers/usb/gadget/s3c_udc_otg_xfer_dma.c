		...
		struct usb_ctrlrequest *crq)
{
	...
	dma_addr = dma_map_single(&dev->dev->dev, &g_status, sizeof(g_status), DMA_TO_DEVICE);
	dma_sync_single_for_device(&dev->dev->dev, dma_addr, sizeof(g_status), DMA_TO_DEVICE);
...
static inline void set_test_mode(struct s3c_udc *dev)
{
	...
	case TEST_PACKET_SEL:
		...
		dma_addr = dma_map_single(&dev->dev->dev, test_pkt, TEST_PKT_SIZE, DMA_TO_DEVICE);
		dma_sync_single_for_device(&dev->dev->dev, dma_addr, TEST_PKT_SIZE, DMA_TO_DEVICE);
