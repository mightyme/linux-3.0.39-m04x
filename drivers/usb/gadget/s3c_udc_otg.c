...
static struct early_suspend usb_resume;
...
static void udc_disable(struct s3c_udc *dev)
{
	...
	if(pdata->phy_power)
		pdata->phy_power(pdev, S5P_USB_PHY_DEVICE, 0);
}
...
static int udc_enable(struct s3c_udc *dev)
{
	...
	if(pdata->phy_power)
		pdata->phy_power(pdev, S5P_USB_PHY_DEVICE, 1);
	enable_irq(dev->irq);
	if (pdata->phy_init)
...
int s3c_vbus_enable(struct usb_gadget *gadget, int is_active)
{
	...
	down(&dev->udc_sem);
	if(atomic_read(&dev->udc_suspended)){
...
static void s3c_udc_soft_connect(void)
{
	...
	__raw_writel(__raw_readl(dev->regs + S3C_UDC_OTG_GINTSTS), dev->regs + S3C_UDC_OTG_GINTSTS);
	__raw_writel(GINTMSK_INIT, dev->regs + S3C_UDC_OTG_GINTMSK);
...
static void usb_resume_func(struct early_suspend *h)
{
	struct s3c_udc *dev = the_controller;
	printk("::::::::::::: %s\n", __func__);
...
static int s3c_udc_probe(struct platform_device *pdev)
{
	...
	atomic_set(&dev->udc_suspended, 0);
	sema_init(&dev->udc_sem, 1);
...
static int s3c_udc_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	down(&dev->udc_sem);
	atomic_set(&dev->udc_suspended, 1);
	if (dev->driver) {
