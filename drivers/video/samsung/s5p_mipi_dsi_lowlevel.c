...
void s5p_mipi_dsi_init_config(struct mipi_dsim_device *dsim)
{
	...
	cfg =(dsim_config->manual_flush << 29) |
...
void s5p_mipi_set_pll_pms(struct mipi_dsim_device *dsim, unsigned int p,
...
{
	unsigned int reg = readl(dsim->reg_base + S5P_DSIM_ESCMODE);
	reg |= (1 << 16);//DSIM_FORCE_BTA;
	writel(reg, dsim->reg_base + S5P_DSIM_ESCMODE);
}

static inline int s5p_mipi_is_state(struct mipi_dsim_device *dsim, /*enum dsim_int_src*/int intsrc)
{   
	int ret = -1;
...
int s5p_mipi_dsi_wait_state(struct mipi_dsim_device *dsim,
...
{
	int reg = readl(dsim->reg_base + S5P_DSIM_MDRESOL);
