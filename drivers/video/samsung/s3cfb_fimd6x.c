...
#ifdef CONFIG_FB_DYNAMIC_FREQ
int s3cfb_frame_adjust(struct s3cfb_global *ctrl, fb_refresh freq)
{
#define MAX_CNT 0xfffff
	...
	if (FB_DYNAMIC_HIGH_FREQ != freq &&
	    FB_DYNAMIC_LOW_FREQ != freq) {
		pr_err("freq %dHZ is out rang\n", freq);
		return -EPERM;
	}
	...
	delta_us = ktime_to_ms(delta_total);
	if (delta_us > delta_max_total || remain_cnt < delta_min_cnt) {
		delta_max_total = delta_us;
		delta_min_cnt= remain_cnt;
		pr_debug("escape time : %Ld mS, remain cnt = %Lu\n", 
			delta_max_total, delta_min_cnt);
	}
	WARN_ON(!remain_cnt);
	return 0;
}
