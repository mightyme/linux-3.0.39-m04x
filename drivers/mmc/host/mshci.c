	...
	struct mmc_data *data)
{
	...
	if (!data->host_cookie) {
		if (host->ops->dma_map_sg_mshci && data->blocks >= 2048) {
			/* if transfer size is bigger than 1MiB */
			host->sg_count = host->ops->dma_map_sg_mshci(host,
				mmc_dev(host->mmc),
				data->sg, data->sg_len, direction, 2);
		} else if (host->ops->dma_map_sg_mshci && data->blocks >= 128) {
...
unmap_entries:
	if (host->ops->dma_unmap_sg_mshci && data->blocks >= 2048) {
		/* if transfer size is bigger than 1MiB */
		host->ops->dma_unmap_sg_mshci(host, mmc_dev(host->mmc),
			data->sg, data->sg_len, direction, 2);
	} else if (host->ops->dma_unmap_sg_mshci && data->blocks >= 128) {
		/* if transfer size is bigger than 64KiB */
		host->ops->dma_unmap_sg_mshci(host, mmc_dev(host->mmc),
			data->sg, data->sg_len, direction, 1);
	} else {
...
//	pr_info("mshci_set_clock: clock = %d, ddr = %d\n", clock, ddr);
//	pr_info("max_clk = %d, div = %d\n", host->max_clk, div);
		...
		if (ios->timing == MMC_TIMING_UHS_DDR50) {
			...
			if (soc_is_exynos4412() || soc_is_exynos4212()) {
				if ((host->max_clk/2) < 46300000) {
					...
					mshci_writel(host, (0x00030003),	/* clock phase shift 135 */
						MSHCI_CLKSEL);
				}
			} else {
							...
							bool is_first_req)
{
	...
	if (host->ops->dma_map_sg_mshci && data->blocks >= 2048) {
		/* if transfer size is bigger than 1MiB */
		sg_count = host->ops->dma_map_sg_mshci(host,
			mmc_dev(host->mmc),
			data->sg, data->sg_len, direction, 2);
	} else if (host->ops->dma_map_sg_mshci && data->blocks >= 128) {
		/* if transfer size is bigger than 64KiB */
...
static void mshci_fifo_init(struct mshci_host *host)
{
	...
	fifo_depth = host->ops->get_fifo_depth(host); //fifo_depth = ((fifo_val & RX_WMARK)>>16)+1;
	fifo_threshold = fifo_depth/2;
	host->fifo_threshold = fifo_threshold;
	host->fifo_depth = fifo_threshold*2;

	pr_debug("%s: FIFO WMARK FOR RX 0x%x WX 0x%x. ###########\n",
