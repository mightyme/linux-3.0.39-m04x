...
mmc_start_request(struct mmc_host *host, struct mmc_request *mrq)
{
	...
	/* blocks is not zeor if MULTIPLE BLOCK WRITE/READ cmd*/
	if (MMC_WRITE_MULTIPLE_BLOCK == mrq->cmd->opcode ||
	    MMC_READ_MULTIPLE_BLOCK == mrq->cmd->opcode) {
		WARN_ON(0 == mrq->data->blocks);
	}
				  ...
				  struct mmc_request *mrq)
{
	 ...
	 * So, needed to change this on exynos4x12 project
	 */
	if (mmc_card_removed(host->card) ||
	   (soc_is_exynos4210() && (!cmd->error || !cmd->retries)))
		return;
