...
{
	pr_debug("%s\n", __func__);
	pr_debug("%s:  MAX: %lu, MIN: %lu\n", __func__, SELFTEST_MAX, SELFTEST_MIN);
	pr_debug("%s:X: %lu\t", __func__, abs(data_st->x - data_nost->x) * CONVERT_TO_MDPS);
	pr_debug("%s:Y: %lu\t", __func__, abs(data_st->y - data_nost->y) * CONVERT_TO_MDPS);
	pr_debug("%s:Z: %lu\t", __func__, abs(data_st->z - data_nost->z) * CONVERT_TO_MDPS);
...
#endif
*/
static int lsm330dlc_gyr_probe(struct i2c_client *client, const struct i2c_device_id *devid)
{
	struct lsm330dlc_gyr_status *stat;
	int err = -1;
	...
	if (client->dev.platform_data == NULL) {
		dev_dbg(&client->dev, "platform_data not provided,no default plaform_data\n");
		goto exit_mutex_unlock;
	} else {
...
static int lsm330dlc_gyr_remove(struct i2c_client *client)
{
	struct lsm330dlc_gyr_status *stat = i2c_get_clientdata(client);
...
#endif
static const struct i2c_device_id lsm330dlc_gyr_id[] = {
	{ LSM330DLC_GYR_DEV_NAME , 0 }, {}, };

MODULE_DEVICE_TABLE(i2c, lsm330dlc_gyr_id);
...
static struct i2c_driver lsm330dlc_gyr_driver = {
	.driver = {
		.owner = THIS_MODULE,
...
static int __init lsm330dlc_gyr_init(void)
{
	...
	return i2c_add_driver(&lsm330dlc_gyr_driver);
}

static void __exit lsm330dlc_gyr_exit(void)
{
	pr_debug("%s exit\n", LSM330DLC_GYR_DEV_NAME);
	i2c_del_driver(&lsm330dlc_gyr_driver);
}
