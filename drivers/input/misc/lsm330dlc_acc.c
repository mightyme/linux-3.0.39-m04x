...
static int lsm330dlc_acc_hw_init(struct lsm330dlc_acc_status *stat)
{
	...
	if (err < 0) {
		dev_dbg(&stat->client->dev, "Error reading WHO_AM_I:"
		...
		int *xyz)
{
	...
	//	pr_info("xyz[0] = %d, xyz[1] = %d xyz[2] = %d\n", xyz[0], xyz[1], xyz[2]);
...
static int selftest_init(struct lsm330dlc_acc_status *acc)
{
	unsigned char buf[5];
	//	pr_info("%s\n", __func__);
	...
	return lsm330dlc_acc_i2c_write(acc, buf, 4);
}

static int selftest_enable(struct lsm330dlc_acc_status *acc)
{
	unsigned char buf[2];
	...
	return lsm330dlc_acc_i2c_write(acc, buf, 1);
}

static void selftest_disable(struct lsm330dlc_acc_status *acc)
{
	unsigned char buf[2];
	...
	/* Disable sensor */
	buf[0] = CTRL_REG1;
	lsm330dlc_acc_i2c_write(acc, buf, 1);
	/* Disable selftest */
	buf[0] = CTRL_REG4;
	lsm330dlc_acc_i2c_write(acc, buf, 1);
}

static int selftest_wait_ZYXDA(struct lsm330dlc_acc_status *acc)
{
	int i, ret;
	...
	for (i = SELFTEST_MEASURE_TIMEOUT; i != 0; i--) {
		...
		ret = lsm330dlc_acc_i2c_read(acc, &data_ready, 1);
		if (ret < 0) {
...
static int selftest_read(struct lsm330dlc_acc_status *acc, int data[3])
{
	int total[3];
	int i, ret;
	...
	for (i = 0; i < SELFTEST_SAMPLES; i++) {
		...
		mutex_lock(&acc->lock);
		ret = lsm330dlc_acc_get_acceleration_data(acc,data);
...
static int lsm330dlc_acc_selftest(struct lsm330dlc_acc_status *acc, int *test_result,
		int self_data[3])
{
	int ret;
	int  data_nost[3];
...
static ssize_t attr_set_Calibration(struct device *dev,
		...
		const char *buf, size_t size)
{
	struct lsm330dlc_acc_status *acc = dev_get_drvdata(dev);
	unsigned long val;
	bool do_calib;
	int err = 0;
	...
	lsm330dlc_acc_calibration(acc, do_calib);
	if (err < 0) {
		...
		const char *buf, size_t size)
{
	...
	while (*calib_value1 && *calib_value1 != ' ')
		++calib_value1;
	*calib_value1 = '\0';
	acc->calib_data[0] = simple_strtol(calib_buf, NULL, 10);
	...
	while (*calib_value1 && *calib_value1 != ' ')
		++calib_value1;
	*calib_value1 = '\0';
	acc->calib_data[1] = simple_strtol(calib_value2, NULL, 10);

	acc->calib_data[2] = simple_strtol(calib_value1+1, NULL, 10);
...
static int create_sysfs_interfaces(struct lsm330dlc_acc_status *acc)
{
	int err;
	...
	acc->accelerometer= class_create(THIS_MODULE, ACC_SYSCLS_NAME);
	if (IS_ERR(acc->accelerometer)) {
		err = PTR_ERR(acc->accelerometer);
		goto exit_class_create_failed;
	}

	acc->class_dev = device_create(
			acc->accelerometer,
			NULL,
			st_acc_device_dev_t,
			acc,
	...
	err = create_sysfs_files(acc->class_dev);
	if (0 > err)
...
exit_device_attributes_create_failed:
	sysfs_remove_link(&acc->class_dev->kobj, device_link_name);
exit_sysfs_create_link_failed:
	device_destroy(acc->accelerometer, st_acc_device_dev_t);
exit_class_device_create_failed:
	acc->class_dev = NULL;
	class_destroy(acc->accelerometer);
...
static void remove_sysfs_interfaces(struct lsm330dlc_acc_status *acc)
{
	if (NULL == acc)
		return;

	if (NULL != acc->class_dev) {
		remove_sysfs_files(acc->class_dev);
		sysfs_remove_link(
				&acc->class_dev->kobj,
				device_link_name);
		acc->class_dev = NULL;
	}
	if (NULL != acc->accelerometer) {
		device_destroy(
				acc->accelerometer,
				st_acc_device_dev_t);
		class_destroy(acc->accelerometer);
		acc->accelerometer= NULL;
	}
}
		...
		const struct i2c_device_id *id)
{
	...
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
		...
		dev_dbg(&client->dev, "client use I2C available\n");
	}
	...
	if (client->dev.platform_data == NULL) {
		err = -EINVAL;
		dev_err(&client->dev, "platform data not passed, and default not provided\n");
		goto err_mutexunlock;
	}
...
MODULE_DESCRIPTION("lsm330dlc accelerometer sysfs driver");
