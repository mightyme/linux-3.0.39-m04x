/*
 * Atmel maXTouch Touchscreen driver for meizu mx
 *
...
static struct mxt_param_object no_charger_param[] = {
	{ 9, 13, 0x01},
	{15,  8, 0x03},
	{46,  2, 0x00},
	{46,  3, 0x00},
...
static struct mxt_param_object charger_param[] = {
	{ 9, 13, 0x01},
	{15,  8, 0x04},
	{46,  2, 0x0C},
	{46,  3, 0x0C},
...
static int mxt_is_charger_inserted(void)
{
	return mx_is_usb_vbus_insert();
}

static int mxt_usb_notifier_event(struct notifier_block *this,
		unsigned long event, void *ptr)
{
	struct mxt_data *data =
		container_of(this, struct mxt_data, usb_notifier);
	int rtn = NOTIFY_DONE;

	if (data->is_suspended) {
		pr_err("%s is_suspended\n", __func__);
		msleep(10);
	}

	pr_info("%s event is USB_VBUS_INSERT:0x%x\n", __func__, event == USB_VBUS_INSERT);
	switch (event) {
	case USB_VBUS_INSERT:
	case USB_VBUS_REMOVE:
		do {
			int charger_status = (event == USB_VBUS_INSERT);
			if(charger_status != data->charger_status) {
				data->charger_status = charger_status;
				schedule_work(&data->charger_work);
			}
		} while (0);
		rtn = NOTIFY_OK;
		break;
...
static int mxt_write_charger_param(struct mxt_data *data,
				int charger_status)
{
	...
	if (charger_status) {
		p = charger_param;
		param_size = sizeof(charger_param) /
			sizeof(struct mxt_param_object);
	} else {
		p = no_charger_param;
		param_size = sizeof(no_charger_param) /
			sizeof(struct mxt_param_object);
	}

	mxt_write_multi_params(data, p, param_size);
...
static void mxt_input_report(struct mxt_data *data, int single_id)
{
	...
	struct input_dev *input_dev = data->ts_dev;
...
{
	u8 status = message->message[0];
	u8 key = message->message[1];

	if (status & 0x80)
		data->key_code = (key==0x1)? KEY_BACK : 
					       (key==0x2)?KEY_MENU :
						KEY_RESERVED;
	data->key_press = (status & 0x80) ?:false;

	input_report_key(data->key_dev, data->key_code, data->key_press);
...
static void charger_work_func(struct work_struct *work)
{
	...
	pr_info("%s, charger_status:%d\n", __func__, data->charger_status);

	mxt_write_charger_param(data, data->charger_status);
}
...
static irqreturn_t mxt_interrupt(int irq, void *dev_id)
{
	...
	do {
		...
		else if (0xc == reportid)
			mxt_input_keyevent(data, &message, id);
		else
...
static int mxt_check_reg_init(struct mxt_data *data)
{
	...
	struct mxt_param_object *mxt_config_param;
	...
	mxt_config_param = kmalloc( sizeof(struct mxt_param_object) 
				* pdata->config_length, GFP_KERNEL);

	if (mxt_config_param == NULL) {
	...
	for (i = 0; i < data->info.object_num; i++) {
		...
		for (j = 0; j < object->size + 1; j++) {
			...
			mxt_config_param[config_offset].type = object->type;
			mxt_config_param[config_offset].index = j;
			mxt_config_param[config_offset].value = pdata->config[config_offset];
		}
		index += object->size + 1;
	}
	mxt_write_multi_params(data, mxt_config_param, pdata->config_length);

	kfree(mxt_config_param);
...
{
	unsigned int max_x = data->pdata->x_size;
	unsigned int max_y = data->pdata->y_size;
...
static ssize_t mxt_debug_show(struct device *dev,
				    struct device_attribute *attr, char *buf)
{
	int count = 0;

	if (machine_is_m030())
		count = sprintf(buf, "irq gpio:%d,INT_USB:%d\n", 
				gpio_get_value(EXYNOS4_GPX1(7)), 
				mxt_is_charger_inserted());
	else
		count = sprintf(buf, "irq gpio:%d,INT_USB:%d\n", 
				gpio_get_value(EXYNOS4_GPX0(1)), 
				mxt_is_charger_inserted());
	return count;
}
		...
		const struct i2c_device_id *id)
{
	...
	struct mxt_data *data;
	struct input_dev *ts_dev, *key_dev;
	int error;
	...
	ts_dev->dev.parent = &client->dev;
	ts_dev->open = mxt_input_open;
	ts_dev->close = mxt_input_close;
	...
	/* For multi touch */
	input_mt_init_slots(ts_dev, MXT_MAX_FINGER);
	input_set_abs_params(ts_dev, ABS_MT_TOUCH_MAJOR,
			     0, MXT_MAX_AREA, 0, 0);
	input_set_abs_params(ts_dev, ABS_MT_POSITION_X,
			     0, data->max_x, 0, 0);
	input_set_abs_params(ts_dev, ABS_MT_POSITION_Y,
			     0, data->max_y, 0, 0);

	input_set_drvdata(ts_dev, data);
	...
	data->charger_status = mxt_is_charger_inserted();
	...
	error = sysfs_create_group(&ts_dev->dev.kobj, &mxt_attr_group);
	if (error)
		goto err_unregister_device;

	key_dev = input_allocate_device();
	if (!key_dev) {
		dev_err(&client->dev, "Failed to allocate input dev\n");
		error = -ENOMEM;
		goto err_remove_group;
	}
	key_dev->name = "mx-touch-keypad";
	key_dev->id.bustype = BUS_I2C;
	key_dev->dev.parent = &client->dev;

	__set_bit(EV_KEY, key_dev->evbit);

	set_bit(KEY_BACK, key_dev->keybit);
	set_bit(KEY_MENU, key_dev->keybit);
	data->key_dev = key_dev;

	error = input_register_device(key_dev);
	if (error)
		goto err_free_keydevice;
	
	mxt_turn_on_calibration(data, 1);
	
	INIT_WORK(&data->charger_work, charger_work_func);
	
	data->usb_notifier.notifier_call = mxt_usb_notifier_event;
	register_mx_usb_notifier(&data->usb_notifier);

#ifdef CONFIG_EARLYSUSPEND
	data->early_suspends.level = EARLY_SUSPEND_LEVEL_BLANK_SCREEN;
...
static int __devexit mxt_remove(struct i2c_client *client)
{
	...
	input_unregister_device(data->ts_dev);
	input_unregister_device(data->key_dev);
	kfree(data->object_table);
	kfree(data);
...
{
	struct mxt_data *data = container_of(h, struct mxt_data, early_suspends);
	struct input_dev *input_dev = data->ts_dev;
	struct mxt_finger *finger = data->finger;
	int id;
	...
	input_report_key(data->key_dev, KEY_BACK, 0);
	input_report_key(data->key_dev, KEY_MENU, 0);
	input_sync(data->key_dev);
