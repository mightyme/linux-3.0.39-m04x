...
#if defined(CONFIG_RTC_BOOT_ALARM)
static int max77686_rtc_set_boot_alarm(struct device *dev,
				      struct rtc_wkalrm *alrm)
{
	...
	if(alrm->enabled) {
		...
		ret = max77686_bulk_read(info->rtc, MAX77686_ALARM2_SEC, RTC_NR_TIME, data);
		if (ret < 0) {
			dev_err(info->dev, "%s: fail to read alarm reg(%d)\n",
					__func__, ret);
			goto out;
		...
		ret = max77686_bulk_write(info->rtc, MAX77686_ALARM2_SEC, RTC_NR_TIME, data);
		if (ret < 0) {
			dev_err(info->dev, "%s: fail to write alarm reg(%d)\n",
					__func__, ret);
			goto out;
...
#if defined(CONFIG_RTC_BOOT_ALARM)
static irqreturn_t max77686_rtc_boot_alarm_irq(int irq, void *data)
{
	struct max77686_rtc_info *info = data;
...
#if defined(CONFIG_RTC_BOOT_ALARM)
	.set_alarm_boot = max77686_rtc_set_boot_alarm,
#endif
};
...
#if defined(CONFIG_RTC_BOOT_ALARM)
	info->boot_alarm_irq = max77686->irq_base + MAX77686_RTCIRQ_RTCA2;
#endif
...
#if defined(CONFIG_RTC_BOOT_ALARM)
	ret = request_threaded_irq(info->boot_alarm_irq, NULL, max77686_rtc_boot_alarm_irq, 0,
			"rtc-alarm1", info);
	if (ret < 0) {
		dev_err(&pdev->dev, "Failed to request boot_alarm IRQ: %d: %d\n",
