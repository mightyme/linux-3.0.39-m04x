...
static void update_packet_state(struct smd_channel *ch)
{
	...
	{
		case IPPROTO_IP:
			printk("%s: IP dummy packet, size=%d!\n", ch->name, ch->current_packet);
			break;
		case IPPROTO_ICMP:
			printk("%s: IP ICMP packet, size=%d!\n", ch->name, ch->current_packet);
			break;			
		case IPPROTO_IGMP:
			printk("%s: IP IGMP packet, size=%d!\n", ch->name, ch->current_packet);
			break;			
		case IPPROTO_IPIP:
			printk("%s: IP IP tunnel packet, size=%d!\n", ch->name, ch->current_packet);
			break;	
		case IPPROTO_TCP:
			printk("%s: IP TCP packet, size=%d!\n", ch->name, ch->current_packet);
			break;			
		case IPPROTO_UDP:
			printk("%s: IP UDP packet, size=%d!\n", ch->name, ch->current_packet);
			break;			
		break;
	}
}
