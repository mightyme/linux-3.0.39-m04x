...
extern struct platform_device *mx_snd_device;
static struct mutex wm8958_setnr_lock;
#endif
...
/*	unsigned int res = 0xFFFFFFFF; */
	...
	if(ret < 0)
	{
		printk("wm8994_write:write failed(R:0x%.4X D:0x%.4X) !!!\n",reg,value);
		return ret;
	}
	...
	if (!wm8994_volatile(codec, reg)) {
		...
		if (ret != 0)
			dev_err(codec->dev, "Cache write to 0x%.4X failed: %d\n",
				reg, ret);
		}
#else
	ret = wm8994_reg_write(codec->control_data, reg, value);
	if(ret < 0)
	{
		printk("wm8994_write:write failed !!! Try again ... \n");
		ret = wm8994_reg_write(codec->control_data, reg, value);
		if(ret < 0)
		{
			printk("wm8994_write:write failed !!!\n");
			return ret;
		}
	}
				...
				unsigned int reg)
{
	...
	ret = wm8994_reg_read(codec->control_data, reg);
	//printk("%s:R:0x%.4X D:0x%.4X\n",__func__,reg,ret);
...
static const struct soc_enum path_enum[] = {
	...
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(string_on_off), string_on_off),
};
extern int set_playback_path(struct snd_soc_codec *codec,u8 playback_path);
extern int set_capture_path(struct snd_soc_codec *codec,u8 capture_path);

static int mx_get_playback_path(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{	
	struct snd_soc_codec *codec = snd_kcontrol_chip (kcontrol);
	struct wm8994_priv *wm8958 = snd_soc_codec_get_drvdata(codec);

  	ucontrol->value.integer.value[0] = wm8958->playback_path;
	return 0;
}

static int mx_set_playback_path(struct snd_kcontrol *kcontrol,
        struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip (kcontrol);
	struct wm8994_priv *wm8958 = snd_soc_codec_get_drvdata(codec);
...
static int mx_get_capture_path(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{	
	struct snd_soc_codec *codec = snd_kcontrol_chip (kcontrol);
	struct wm8994_priv *wm8958 = snd_soc_codec_get_drvdata(codec);

  	ucontrol->value.integer.value[0] = wm8958->capture_path;
	return 0;
}

static int mx_set_capture_path(struct snd_kcontrol *kcontrol,
        struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip (kcontrol);
	struct wm8994_priv *wm8958 = snd_soc_codec_get_drvdata(codec);
...
#define	WM8958_WORK_SET_NR
#ifdef	WM8958_WORK_SET_NR
#define ABS(x)		((x) < 0 ? (-x) : (x))
static void wm8958_noise_set_func(struct work_struct *work);
static DECLARE_WORK(wm8958_noise_set_work,wm8958_noise_set_func);
static void wm8958_noise_set_func(struct work_struct *work)
{
	struct snd_soc_codec *codec;
	struct wm8994_priv *wm8958;
	unsigned short val;
	int i,steps,dir;
	...
	codec = wm8994_codec;	
	wm8958 = snd_soc_codec_get_drvdata(codec);
	
	if(wm8958->music_headset_nrec_switch)
	{
	  val = VOL_NORMAL_HP;
	  dir = -2;	  
	}
	...
	{
	  val = VOL_NORMAL_HP_NR;
	  dir = 2;	  	  
	}	
	
	steps = ABS(VOL_NORMAL_HP - VOL_NORMAL_HP_NR)/2;
	
	for (i=0; i< steps; i++) 
	{
		val += dir;
		snd_soc_update_bits (codec, WM8994_LEFT_OUTPUT_VOLUME, WM8994_HPOUT1L_VOL_MASK|WM8994_HPOUT1_VU_MASK, val);
		snd_soc_update_bits (codec, WM8994_RIGHT_OUTPUT_VOLUME, WM8994_HPOUT1R_VOL_MASK|WM8994_HPOUT1_VU_MASK, val);	
...
/*
 *----------------------------------------------------------------------------
 * Function : mx_get_noise_setting
 * Purpose  : Callback to get the value of a double mixer control that spans
 *            two registers.
 *
 *----------------------------------------------------------------------------
 */
int mx_get_noise_setting (struct snd_kcontrol *kcontrol,
			    struct snd_ctl_elem_value *ucontrol)
{
  struct snd_soc_codec *codec = snd_kcontrol_chip (kcontrol);
  struct wm8994_priv *wm8958 = snd_soc_codec_get_drvdata(codec);
  
  if(wm8958->playback_path == PLAYBACK_SPK_HP_NORMAL  
  	||  wm8958->playback_path == PLAYBACK_HP_NORMAL )
  	ucontrol->value.integer.value[0] = wm8958->music_headset_nrec_switch;
  else
 ...
 */
int
mx_set_noise_setting (struct snd_kcontrol *kcontrol,
			    struct snd_ctl_elem_value *ucontrol)
{
  struct snd_soc_codec *codec = snd_kcontrol_chip (kcontrol);
  struct wm8994_priv *wm8958 = snd_soc_codec_get_drvdata(codec);
  int on= ucontrol->value.integer.value[0];
  int err = 0;
  
#ifndef WM8958_WORK_SET_NR  
  int reg = WM8994_LEFT_OUTPUT_VOLUME;
  int reg2 = WM8994_RIGHT_OUTPUT_VOLUME;
  unsigned short val;
  ...
  val_mask = WM8994_HPOUT1L_VOL_MASK|WM8994_HPOUT1_VU_MASK;
  ...
  err =snd_soc_update_bits (codec, reg, WM8994_HPOUT1_VU_MASK, WM8994_HPOUT1_VU);
  err =snd_soc_update_bits (codec, reg2, WM8994_HPOUT1_VU_MASK, WM8994_HPOUT1_VU);	
  wm8958->music_headset_nrec_switch = on;  
  
#else
  wm8958->music_headset_nrec_switch = on;  
  schedule_work(&wm8958_noise_set_work);
#endif
...
static void wm8958_ext_mic_bias_func(struct work_struct *work);
static DECLARE_WORK(ext_mic_bias_work,wm8958_ext_mic_bias_func);
static void wm8958_ext_mic_bias_func(struct work_struct *work)
{
	struct snd_soc_codec *codec;
	...
	if(g_bBiasOnOff)
		snd_soc_update_bits(codec, WM8994_POWER_MANAGEMENT_1,WM8994_MICB2_ENA_MASK, WM8994_MICB2_ENA);
	else
		snd_soc_update_bits(codec, WM8994_POWER_MANAGEMENT_1,WM8994_MICB2_ENA_MASK, 0);
...
void set_ext_mic_bias(bool bOnOff)
{
	if(!wm8994_codec)
		return;
	if(wm8994_codec->dapm.bias_level == SND_SOC_BIAS_OFF)
		return;
	g_bBiasOnOff = bOnOff;
...
static ssize_t wm8958_show_property(struct device *dev,
                                      struct device_attribute *attr,
                                      char *buf);
static ssize_t wm8958_store(struct device *dev, 
			     struct device_attribute *attr,
...
static struct device_attribute wm8958_attrs[] = {
    WM8958_CODEC_ATTR(loopback_aif2),
    WM8958_CODEC_ATTR(reg_program),
    WM8958_CODEC_ATTR(aif2_switch),
};
enum {
    WM8958_LOOPBACK_AIF2 = 0,
    WM8958_REG_PROGRAM,		
    WM8958_AIF2_SWITCH,		
};
static ssize_t wm8958_show_property(struct device *dev,
                                      ...
                                      char *buf)
{
	...
	case WM8958_REG_PROGRAM:
		...
		break;
	case WM8958_AIF2_SWITCH:
		i += scnprintf(buf+i, PAGE_SIZE-i, "Can't be Read\n");
		break;
	default:
			     ...
			     const char *buf, size_t count)
{
	...
	case WM8958_LOOPBACK_AIF2:
		if (sscanf(buf, "%d\n", &value) == 1) {			
			if(value)
				snd_soc_update_bits(codec, WM8994_AIF2_CONTROL_2,WM8994_AIF2_LOOPBACK_MASK, WM8994_AIF2_LOOPBACK);
			else
				snd_soc_update_bits(codec, WM8994_AIF2_CONTROL_2,WM8994_AIF2_LOOPBACK_MASK, 0x00);

			printk ("<<<<<<<<<<<<<<<<<<<<<<<<< Set loopback_AIF2: %d >>>>>>>>>>>>>>>>>>>>>>>>>>>>>\n",value);
		}
		ret = count;
		break;		
	case WM8958_REG_PROGRAM:
		...
		{
			value = snd_soc_write(codec, reg, value);
		}
		ret = count;
		break;		
	case WM8958_AIF2_SWITCH:
...
static int wm8958_create_attrs(struct device * dev)
{
	int i, rc;

	for (i = 0; i < ARRAY_SIZE(wm8958_attrs); i++) {
		rc = device_create_file(dev, &wm8958_attrs[i]);
		if (rc)
			goto wm8958_attrs_failed;
	}
	goto succeed;
...
wm8958_attrs_failed:
	printk(KERN_INFO "%s(): failed!!!\n", __func__);	
	while (i--)
		device_remove_file(dev, &wm8958_attrs[i]);
succeed:		
	return rc;

}

static void wm8958_destroy_atts(struct device * dev)
{
	int i;
	for (i = 0; i < ARRAY_SIZE(wm8958_attrs); i++)
		device_remove_file(dev, &wm8958_attrs[i]);
...
#ifdef CONFIG_SND_SOC_MX_WM8958	       
SOC_SINGLE("MIC Bias1 Switch", WM8994_POWER_MANAGEMENT_1, 4, 1, 0),
SOC_SINGLE("MIC Bias2 Switch", WM8994_POWER_MANAGEMENT_1, 5, 1, 0),
SOC_SINGLE("AIF1 Loopback", WM8994_AIF1_CONTROL_2, 0, 1, 0),
SOC_SINGLE("AIF2 Loopback", WM8994_AIF2_CONTROL_2, 0, 1, 0),
SOC_ENUM_EXT("Playback_path Selection", path_enum[0],mx_get_playback_path, mx_set_playback_path),
SOC_ENUM_EXT("Capture_path Selection", path_enum[1],mx_get_capture_path, mx_set_capture_path),
SOC_ENUM_EXT("Noise Suppress Mode", path_enum[2],mx_get_noise_setting, mx_set_noise_setting),
#endif
};
			  ...
			  unsigned int freq_in, unsigned int freq_out)
{
	...
	// Add by Chwei 2012.1.6
	// Remove the noise when play ultrasonic ringtones(up to around 14.1kHz).
	if(id == 0)
	snd_soc_update_bits(codec, WM8994_FLL1_CONTROL_4 + reg_offset,
			    WM8994_FLL1_N_MASK|WM8994_FLL1_LOOP_GAIN_MASK,
				    (fll.n << WM8994_FLL1_N_SHIFT) |(5 << WM8994_FLL1_LOOP_GAIN_SHIFT));
	else
	snd_soc_update_bits(codec, WM8994_FLL1_CONTROL_4 + reg_offset,
			    WM8994_FLL1_N_MASK,
				 ...
				 enum snd_soc_bias_level level)
{
	...
	case SND_SOC_BIAS_OFF:
		if (codec->dapm.bias_level == SND_SOC_BIAS_STANDBY) {
			...
			snd_soc_update_bits(codec, WM8994_ANTIPOP_2,
					    WM8994_BIAS_SRC_MASK|
					    WM8994_STARTUP_BIAS_ENA_MASK|
					    WM8994_VMID_BUF_ENA_MASK|
...
{
	struct snd_soc_codec *codec = dai->codec;
	wm8994_hw_params(substream,params,dai);
		
	snd_soc_update_bits(codec, WM8994_FLL2_CONTROL_1,WM8994_FLL2_ENA_MASK, WM8994_FLL2_ENA);
	snd_soc_update_bits(codec, WM8994_AIF1_CLOCKING_1,WM8994_AIF1CLK_ENA_MASK, WM8994_AIF1CLK_ENA);
	...
	snd_soc_update_bits(codec, WM8994_AIF2_CLOCKING_1,WM8994_AIF2CLK_ENA_MASK, WM8994_AIF2CLK_ENA);
	snd_soc_update_bits(codec, WM8994_AIF2_DAC_FILTERS_1,WM8994_AIF2DAC_MONO_MASK, WM8994_AIF2DAC_MONO);	
	snd_soc_update_bits(codec, WM8994_AIF2_CONTROL_2,WM8994_AIF2_MONO_MASK, 0);     
	...
	snd_soc_update_bits(codec, WM8994_POWER_MANAGEMENT_5,
			WM8994_AIF2DACR_ENA_MASK|WM8994_AIF2DACL_ENA_MASK, WM8994_AIF2DACR_ENA|WM8994_AIF2DACL_ENA);
	msleep(20);
...
struct snd_soc_dai_driver wm8994_dai[] = {
	...
	},
	{
		...
		.playback = {
			...
			.channels_min = 1,
			.channels_max = 2,
			.rates = SNDRV_PCM_RATE_8000,//WM8994_RATES,
			.formats = WM8994_FORMATS,
		},
		.capture = {
...
int wm8994_isalwayson(struct snd_soc_codec *codec)
{	
	...
	if(wm8994 == NULL)
		return 0;
	//printk("%s:path = %d\n",__func__,wm8994->playback_path);
	if((wm8994->playback_path >= PLAYBACK_SPK_INCALL 
		&& wm8994->playback_path <= PLAYBACK_SPK_HP_INCALL)
		|| wm8994->playback_path == PLAYBACK_BT_INCALL)
	{
			return 1;
	}
	return 0;
...
int wm8994_get_playback_path(struct snd_soc_codec *codec) 
{     
    struct wm8994_priv *wm8994; 
...
static int wm8994_codec_probe(struct snd_soc_codec *codec)
{
	...
	snd_soc_update_bits(codec, WM8994_SPEAKER_VOLUME_LEFT,
				WM8994_SPKOUT_VU_MASK,WM8994_SPKOUT_VU);
	snd_soc_update_bits(codec, WM8994_SPEAKER_VOLUME_RIGHT,
				WM8994_SPKOUT_VU_MASK,WM8994_SPKOUT_VU);
	snd_soc_update_bits(codec, WM8994_LEFT_OUTPUT_VOLUME,
				WM8994_HPOUT1_VU_MASK,WM8994_HPOUT1_VU);
	snd_soc_update_bits(codec, WM8994_RIGHT_OUTPUT_VOLUME,
				WM8994_HPOUT1_VU_MASK,WM8994_HPOUT1_VU);
	snd_soc_update_bits(codec, WM8994_LEFT_OPGA_VOLUME,
				WM8994_MIXOUT_VU_MASK,WM8994_MIXOUT_VU);
	snd_soc_update_bits(codec, WM8994_RIGHT_OPGA_VOLUME,
				WM8994_MIXOUT_VU_MASK,WM8994_MIXOUT_VU);
	...
	// Connect IN1RN to IN1R PGA, Connect IN1RP to IN1R PGA
	// Connect IN1LN to IN1L PGA, Connect IN1LP to IN1L PGA
	// Connect IN2LN to IN2L PGA, Connect IN2LP to IN2L PGA
	snd_soc_write(codec, WM8994_INPUT_MIXER_2, 0xF3);

	//snd_soc_write(codec, WM8958_MICBIAS1, 0x23);//1.8v   (default 0x39 => AVDD1=3.0v)
	//snd_soc_write(codec, WM8958_MICBIAS2, 0x23);//1.8v   (default 0x39 => AVDD1=3.0v)

	if(!machine_is_m030())
		snd_soc_write(codec, WM8958_MICBIAS2, 0x2E);//2.6v   (default 0x39 => AVDD1=3.0v)

	// 01 = Voice mode 1 ( fc = 127Hz at fs =8kHz )
	snd_soc_update_bits(codec, WM8994_AIF2_ADC_FILTERS,WM8994_AIF2ADC_HPF_CUT_MASK|WM8994_AIF2ADCL_HPF_MASK|WM8994_AIF2ADCR_HPF_MASK, (1<<13)|WM8994_AIF2ADCL_HPF|WM8994_AIF2ADCR_HPF);// -6dB


	// GPIO1
	snd_soc_write(codec, WM8994_GPIO_1, 0x8101);
	snd_soc_write(codec, WM8994_GPIO_6, 0x8101);

	// AIF3
	snd_soc_write(codec, WM8994_GPIO_8, 0xA100 );
	snd_soc_write(codec, WM8994_GPIO_9, 0xA100 );
	snd_soc_write(codec, WM8994_GPIO_10, 0xA100 );
	snd_soc_write(codec, WM8994_GPIO_11, 0xA100 );
	
	// 16bits DSP Mode 
	snd_soc_update_bits(codec, WM8958_AIF3_CONTROL_1,
		WM8958_AIF3_LRCLK_INV_MASK|WM8958_AIF3_WL_MASK|WM8958_AIF3_FMT_MASK,
		(0<<WM8958_AIF3_WL_SHIFT)|(3<<WM8958_AIF3_FMT_SHIFT));
	
	snd_soc_update_bits(codec, WM8958_AIF3_CONTROL_2,WM8958_AIF3DAC_BOOST_MASK,(2<<WM8958_AIF3DAC_BOOST_SHIFT));// boost +12dB
		

	// Mixer out
	snd_soc_update_bits(codec, WM8994_OUTPUT_MIXER_1,WM8994_DAC1L_TO_MIXOUTL_MASK, WM8994_DAC1L_TO_MIXOUTL);//
	snd_soc_update_bits(codec, WM8994_OUTPUT_MIXER_2,WM8994_DAC1R_TO_MIXOUTR_MASK, WM8994_DAC1R_TO_MIXOUTR);// 
	
	snd_soc_update_bits(codec, WM8994_DAC1_LEFT_MIXER_ROUTING, WM8994_AIF1DAC1L_TO_DAC1L_SHIFT, WM8994_AIF1DAC1L_TO_DAC1L);
	snd_soc_update_bits(codec, WM8994_DAC1_RIGHT_MIXER_ROUTING, WM8994_AIF1DAC1R_TO_DAC1R_SHIFT, WM8994_AIF1DAC1R_TO_DAC1R);

	// SPK Mixer
	//snd_soc_write(codec, WM8994_SPEAKER_MIXER, WM8994_DAC1L_TO_SPKMIXL|WM8994_DAC1R_TO_SPKMIXR);
	//snd_soc_write(codec, WM8994_SPEAKER_MIXER, WM8994_MIXOUTL_TO_SPKMIXL|WM8994_MIXOUTR_TO_SPKMIXR);
	snd_soc_write(codec, WM8994_SPKMIXR_ATTENUATION,0x0000);// +0dB  Class D
	snd_soc_write(codec, WM8994_SPKMIXL_ATTENUATION,0x0000);//+0dB
	snd_soc_write(codec, WM8994_SPKOUT_MIXERS, WM8994_SPKMIXL_TO_SPKOUTL|WM8994_SPKMIXR_TO_SPKOUTL);
	
#ifdef	WM8958_WORK_SET_NR	
		mutex_init(&wm8958_setnr_lock);
#endif
...
#ifdef CONFIG_SND_SOC_MX_WM8958
	if(mx_snd_device)
		wm8958_create_attrs(&mx_snd_device->dev);
#endif
	return 0;
...
#ifdef CONFIG_SND_SOC_MX_WM8958
	if(mx_snd_device)
		wm8958_destroy_atts(&mx_snd_device->dev);
#endif
	wm8994_set_bias_level(codec, SND_SOC_BIAS_OFF);
