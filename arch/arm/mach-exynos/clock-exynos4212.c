...
static int exynos4212_clk_ip_maudio_ctrl(struct clk *clk, int enable)
{
...
static struct clk exynos4212_init_clocks_off[] = {
	...
	{
		.name		= "clk_audioss",
		.enable		= exynos4212_clk_ip_maudio_ctrl,
		.ctrlbit		= (1 << 0),
	}, {
		.name		= "clk_imem",
		.enable		= exynos4212_clk_ip_maudio_ctrl,
...
/* for trustzone */
static struct clk exynos4212_tz_clocks[] = {
	/* CLK_GATE_IP_DMC1 */
	{
		.name		= "tzasc_rw",
...
static void exynos4212_clock_resume(void)
{
	...
	do {
		tmp = __raw_readl(EXYNOS4_EPLL_CON0);
	} while (!(tmp & 0x1 << EXYNOS4_EPLLCON0_LOCKED_SHIFT) && --timeout);

	timeout = 5000;
	do {
		tmp = __raw_readl(EXYNOS4_VPLL_CON0);
	} while (!(tmp & 0x1 << EXYNOS4_VPLLCON0_LOCKED_SHIFT) && --timeout);
...
void __init exynos4212_register_clocks(void)
{
	...
	s3c_register_clocks(exynos4212_tz_clocks, ARRAY_SIZE(exynos4212_tz_clocks));
	if (!trustzone_on())
		s3c_disable_clocks(exynos4212_tz_clocks, ARRAY_SIZE(exynos4212_tz_clocks));
