...
static struct clk exynos4_init_clocks_off[] = {
	...
	{
		.name		= "clk_ppmu",
		.devname	= PPMU_CLOCK_NAME(bus_left, 13),
	...
	/*  CLK_GATE_IP_IMAGE */	
	{
		...
		.name		= "clk_ppmu",
		.devname	= PPMU_CLOCK_NAME(image, 7),
		.enable		= exynos4_clk_ip_image_ctrl,
		.ctrlbit	= (1 << 9),
	},
	...
	{
		.name		= "clk_ppmu",
		.devname	= PPMU_CLOCK_NAME(bus_right, 14),
	...
	/*  CLK_GATE_IP_CAM */
	{
		...
		.name		= "clk_ppmu",
		.devname	= PPMU_CLOCK_NAME(camif, 2),
		.enable		= exynos4_clk_ip_cam_ctrl,
		.ctrlbit	= (1 << 16),
	}, {
		.name		= "pxl_async0",
	...
	/*  CLK_GATE_IP_MFC  */
	{
		...
		.enable		= exynos4_clk_ip_mfc_ctrl,
		.ctrlbit	= (1 << 1),
		.parent        	= &exynos4_init_clocks_off[42],
	}, {
		.name		= "clk_sysmmu",
		...
		.name		= "clk_ppmu",
		.devname	= PPMU_CLOCK_NAME(mfc_l, 10),
		.enable		= exynos4_clk_ip_mfc_ctrl,
		.ctrlbit	= (1 << 3),
	}, {
		.name		= "clk_ppmu",
		.devname	= PPMU_CLOCK_NAME(mfc_r, 11),
		.enable		= exynos4_clk_ip_mfc_ctrl,
		.ctrlbit	= (1 << 4),
	}, 
	/*  CLK_GATE_IP_G3D */
	...
	/* CLK_GATE_IP_DMC */
	{
		...
		.name		= "clk_ppmu",
		.devname	= PPMU_CLOCK_NAME(dmc_l, 4),
		.enable		= exynos4_clk_ip_dmc_ctrl,
		.ctrlbit	= (1 << 8),
	}, {
		.name		= "clk_ppmu",
		.devname	= PPMU_CLOCK_NAME(dmc_r, 5),
		.enable		= exynos4_clk_ip_dmc_ctrl,
		.ctrlbit	= (1 << 9),
	}, {
		.name		= "clk_ppmu",
		.devname	= PPMU_CLOCK_NAME(cpu, 3),
		.enable		= exynos4_clk_ip_dmc_ctrl,
		.ctrlbit	= (1 << 10),
	}, {
		.name		= "clk_sysmmu",
...
static struct clk exynos4_tz_clocks[] = {
	{
		.name		= "tzpc",
...
static struct clksrc_clk exynos4_clk_dout_mipiphy = {
	.clk	= {
		.name		= "sclk_mipi_phy",
...
static struct clksrc_clk exynos4_clksrcs[] = {
	{
		...
		.clk	= {
			.name		= "sclk_mipi",
			.devname		= "s5p-mipi-dsim.0",
			.parent		= &exynos4_clk_dout_mipiphy.clk,
		},
...
void __init_or_cpufreq exynos4_setup_clocks(void)
{
	...
	pr_debug("EXYNOS4: PLL settings, A=%ld, M=%ld, E=%ld V=%ld",
			apll, mpll, epll, vpll);
	...
	pr_debug("EXYNOS4: ARMCLK=%ld, DMC=%ld, ACLK200=%ld\n"
	...
	if(soc_is_exynos4212() || soc_is_exynos4412() )
		pr_debug("EXYNOS4: ACLK400=%ld ACLK266=%ld\n",
...
void __init exynos4_register_clocks(void)
{
	...
	s3c_register_clocks(exynos4_tz_clocks, ARRAY_SIZE(exynos4_tz_clocks));
	if (!trustzone_on())
		s3c_disable_clocks(exynos4_tz_clocks, ARRAY_SIZE(exynos4_tz_clocks));
