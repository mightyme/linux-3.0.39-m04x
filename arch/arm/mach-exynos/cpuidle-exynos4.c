...
#else
static inline int bt_uart_rts_ctrl(int on) {return 0;}
#endif
...
static unsigned int idle_abb = ABB_MODE_075V;
module_param_named(idle_abb, idle_abb, uint, 0644);
				    ...
				    struct cpuidle_state *state)
{
	...
	void __iomem *directgo_addr;
	void __iomem *directgo_flag;
	...
	if (trustzone_on()) {
		directgo_addr = S5P_VA_SYSRAM_NS + 0x24;
		directgo_flag = S5P_VA_SYSRAM_NS + 0x20;
	} else {
		if (soc_is_exynos4210()) {
			directgo_addr = S5P_INFORM7;
			directgo_flag = S5P_INFORM6;
		} else {
			directgo_addr = S5P_VA_SYSRAM + 0x24;
			directgo_flag = S5P_VA_SYSRAM + 0x20;
		}
	}

	__raw_writel(virt_to_phys(exynos4_idle_resume), directgo_addr);
	__raw_writel(0xfcba0d10, directgo_flag);
	...
	if (!soc_is_exynos4210()) {
		...
		exynos4x12_set_abb_member(ABB_ARM, idle_abb);
	}
				   ...
				   struct cpuidle_state *state)
{
	...
	if (!soc_is_exynos4210()) {
		...
		exynos4x12_set_abb_member(ABB_ARM, idle_abb);
		exynos4x12_set_abb_member(ABB_INT, idle_abb);
	}
...
static int __maybe_unused exynos4_enter_lowpower(struct cpuidle_device *dev,
				  struct cpuidle_state *state)
{
	...
	if (new_state == &dev->states[0]) {
		idle_time = exynos4_enter_idle(dev, new_state);
	} else {
