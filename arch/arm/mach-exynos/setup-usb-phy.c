...
static DEFINE_SEMAPHORE(phy_lock);
static struct clk *phy_clk = NULL;
static int enter_lpa_flag=0;
...
static void exynos_usb_mux_change(struct platform_device *pdev, int val)
{
	...
	if (is_host != val)
		dev_info(&pdev->dev, "Change USB MUX from %s to %s\n",
			is_host ? "Host" : "Device",
			val ? "Host" : "Device");
}
...
static int exynos4_usb_host_is_suspended(void)
{
	return (readl(EXYNOS4_PHYPWR) & PHY1_STD_FORCE_SUSPEND) ? 1 : 0;
}
...
int exynos4_check_usb_op(void)
{
	...
	if (exynos4_usb_device_phy_is_on()) {
		op = 1;
		goto done;
	}else if(!exynos4_usb_host_is_suspended()){
		op = 1;
		goto done;
...
static int exynos4_usb_phy20_init(struct platform_device *pdev)
{
	...
	dev_info(&pdev->dev, "power on usb PHY\n");
	/*
	 *  set XuhostOVERCUR to in-active by controlling ET6PUD[15:14]
	 *  0x0 : pull-up/down disabled
	 *  0x1 : pull-down enabled
...
static int exynos4_usb_dev_phy20_init(struct platform_device *pdev)
{
	u32 phypwr, phyclk, rstcon;
...
static int exynos4_usb_dev_phy20_exit(struct platform_device *pdev)
{
	u32 rstcon;
...
static int exynos_usb_dev_phy20_init(struct platform_device *pdev)
{
	...
	if (soc_is_exynos4212() || soc_is_exynos4412())
		err = exynos4_usb_dev_phy20_init(pdev);
	else
		err = exynos5_usb_phy20_init(pdev);
...
static int exynos_usb_dev_phy20_exit(struct platform_device *pdev)
{
	...
	if (soc_is_exynos4212() || soc_is_exynos4412())
		exynos4_usb_dev_phy20_exit(pdev);
	else
...
{
	u32 rstcon, hsic_ctrl, phypwr, phyclk;
